...
valid_mask = np.array([(w.lower() in tr_words) for w in vocab], dtype=bool)
print(f"Strict Turkish mask count: {valid_mask.sum()} / {len(vocab)}")
        ...
        if next_counts:
            ...
            for nxt_id, count in next_counts.items():
                if nxt_id < len(logits) and valid_mask[nxt_id]:
                    logits[nxt_id] += 30.0 * (count / total)
