...
RIDM_EXPORT void ridm_cosine_matvec_f32(const float* matrix, const float* vector, const float* row_norms, float vec_norm, float* out, int64_t rows, int cols) {
    if (!matrix || !vector || !out || rows <= 0 || cols <= 0) return;
    float inv_vec_norm = (vec_norm > 1e-8f) ? (1.0f / vec_norm) : 1.0f;
#pragma omp parallel for schedule(static) if(rows > 64)
    for (int64_t row = 0; row < rows; ++row) {
        float d = dot(matrix + row * static_cast<int64_t>(cols), vector, cols);
        float rn = row_norms ? row_norms[row] : 1.0f;
        float inv_rn = (rn > 1e-8f) ? (1.0f / rn) : 1.0f;
        out[row] = d * inv_rn * inv_vec_norm;
    }
}
...
RIDM_EXPORT int64_t ridm_sample_logits_constrained_f32(
    const float* logits, const uint8_t* valid_mask, int64_t v_size,
    const int64_t* ngram_boost_ids, const float* ngram_boost_vals, int ngram_count,
    const int64_t* penalty_ids, int penalty_count, float penalty_val,
...
#pragma omp parallel for schedule(static) if(v_size > 512)
    for (int64_t i = 0; i < v_size; ++i) {
        ...
        if (valid_mask && !valid_mask[i]) {
            score -= 1000.0f;
        }
        candidates[i] = {score, i};
    }

    if (ngram_boost_ids && ngram_boost_vals && ngram_count > 0) {
        for (int i = 0; i < ngram_count; ++i) {
            int64_t id = ngram_boost_ids[i];
            if (id >= 0 && id < v_size) {
                candidates[id].first += ngram_boost_vals[i];
            }
        }
    }

    if (penalty_ids && penalty_count > 0) {
        for (int i = 0; i < penalty_count; ++i) {
            int64_t id = penalty_ids[i];
            if (id >= 0 && id < v_size) {
                candidates[id].first -= penalty_val;
            }
        }
    }
    ...
    for (int i = 0; i < top_k; ++i) {
        candidates[i].first /= temperature;
    }
